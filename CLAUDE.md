...
- **`routers/enrollments.py`** — the paid funnel. `require_admin(x_admin_key)` is the shared gate (header must equal `ADMIN_KEY`; the older `verify` handler still inlines the same check).
  - `POST /api/enrollments` — public. Records a pending enrollment with the programme, expected amount, referral code/discount, and a user-supplied bank **transfer reference**. Status starts `pending_verification`. Queues (FastAPI `BackgroundTasks`, after the response) the "received" email to the student **then** the `send_academy_notification_email` heads-up to `ACADEMY_NOTIFICATION_EMAIL`.
...
- **`services/email_service.py`** — thin Resend REST wrapper; all four senders POST through one `_send_via_resend(payload)` helper that no-ops (returns `False`, prints) when `RESEND_API_KEY` is unset, swallows `RequestException`, **retries on `429`** (Resend free tier ≈ 2 req/s, honours `Retry-After`, up to 4 attempts), and logs the response body on other non-2xx (e.g. the `403` you get sending to a non-owner address before a domain is verified). Email is always best-effort and never blocks the DB write. The confirmation email appends the WhatsApp group link for the enrollment's `programme_key` (fallback `PAID_STUDENTS_WHATSAPP_LINK`); `send_enrollment_verified_email` takes `programme_key` for this, and the `verify` query's `RETURNING` was extended to supply it. **Don't touch the user-facing `send_welcome_email` / `send_enrollment_*` copy without a reason.**
